/* global EventEmitter, Videos, io, Peer */
'use strict';
...
ChatProxy.prototype.onMessage = function (cb) {
  this.addListener(Videos.USER_MESSAGE, cb);
};
...
ChatProxy.prototype.onUserConnected = function (cb) {
  this.addListener(Videos.USER_CONNECTED, cb);
};

ChatProxy.prototype.onUserDisconnected = function (cb) {
  this.addListener(Videos.USER_DISCONNECTED, cb);
};
...
ChatProxy.prototype.connect = function (username) {
  ...
  this.socket.on('connect', function () {
    self.socket.on(Videos.USER_CONNECTED, function (userId) {
      if (userId === self.getUsername()) {
        return;
      }
      self._connectTo(userId);
      self.emit(Videos.USER_CONNECTED, userId);
      console.log('User connected', userId);
    });
    self.socket.on(Videos.USER_DISCONNECTED, function (userId) {
      if (userId === self.getUsername()) {
        return;
      }
      self._disconnectFrom(userId);
      self.emit(Videos.USER_DISCONNECTED, userId);
  ...
  this.peer.on('connection', function (conn) {
    ...
    self.emit(Videos.USER_CONNECTED, conn.peer);
  });
};
...
ChatProxy.prototype._registerPeer = function (username, conn) {
  ...
  conn.on('data', function (msg) {
    ...
    this.emit(Videos.USER_MESSAGE, { content: msg, author: username });
  }.bind(this));
};
